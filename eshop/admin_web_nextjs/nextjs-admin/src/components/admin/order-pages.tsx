...
export function OrderDetailPage({ id }: { id: string }) {
  ...
  return <>
    ...
    <div className="grid gap-6 xl:grid-cols-3">
      ...
      <section className={`${card} xl:col-span-3`}><h2 className="mb-4 text-lg font-semibold text-dark dark:text-white">Product items</h2><DataTable columns={["Product snapshot", "Quantity", "Unit price", "Discount", "Total", "Trader", "Branch", "Note"]} rows={order.items} empty="No items." render={item => { const image = resolveMediaUrl(item.product_media_url); return [<div className="flex min-w-52 items-start gap-3">{image ? <img src={image} alt={item.product_name_snapshot} className="h-10 w-10 rounded object-cover" /> : <div className="h-10 w-10 rounded bg-gray-2 dark:bg-dark-2" />}<span>{item.product_name_snapshot}<br /><span className="text-xs">{item.product_sku_snapshot || item.product_id_snapshot || (item.product ? `Product #${item.product}` : "Deleted or custom product")}</span>{item.selected_specifications_snapshot.length ? <span className="mt-1 block text-xs text-gray-500">{item.selected_specifications_snapshot.map(specification => `${specification.group_name}: ${specification.value}`).join(" · ")}</span> : null}</span></div>, item.quantity, money(item.unit_price, order.currency), money(item.line_discount, order.currency), money(item.line_total, order.currency), item.trader_name_snapshot || "-", item.branch_name_snapshot || "-", item.note || "-"]; }} /></section>
