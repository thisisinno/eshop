...
export interface ProductCategory { id: number; name: string; slug: string; parent: number | null; description: string; icon: string; image: string | null; display_order: number; is_featured: boolean; is_active: boolean; created_at: string; updated_at: string; }
export type ProductMediaType = "image" | "clip" | "spin_frame" | "model_3d" | "poster";
...
export interface BrandStatus { id: number; media: string; media_url: string | null; media_type: "image" | "video"; caption: string; display_duration_seconds: number; is_active: boolean; starts_at: string; expires_at: string | null; sort_order: number; viewer_count: number; total_views: number; created_at: string; updated_at: string; }
export interface BrandStatusViewer { id: number; viewer_name: string; viewer_type: "user" | "anonymous"; first_viewed_at: string; last_viewed_at: string; view_count: number; }
