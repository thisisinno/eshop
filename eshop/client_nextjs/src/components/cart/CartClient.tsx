...
export function CartClient({ initialCart }: { initialCart: Cart }) {
  ...
  return (
    <section>
      <h1 className="mb-4 text-2xl font-black md:text-3xl">Cart</h1>
      <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
        <div className="space-y-3">
          {cart.items.map((item) => {
            const image = resolveMediaUrl(item.product.primary_media_url);
            return (
              <article key={item.id} className="grid grid-cols-[86px_1fr] gap-3 rounded-lg border border-[var(--color-border)] bg-white p-3 shadow-sm">
                <Link href={`/products/${item.product.id}`} className="relative aspect-square overflow-hidden rounded-lg bg-slate-100">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex h-10 items-center rounded-lg border border-[var(--color-border)]">
                        <button disabled={busy === item.id} aria-label="Decrease quantity" onClick={() => updateItem(item.id, item.quantity - 1)} className="grid h-10 w-10 place-items-center"><Minus aria-hidden className="h-4 w-4" /></button>
                        <span className="w-8 text-center text-sm font-bold">{item.quantity}</span>
                        <button disabled={busy === item.id} aria-label="Increase quantity" onClick={() => updateItem(item.id, item.quantity + 1)} className="grid h-10 w-10 place-items-center"><Plus aria-hidden className="h-4 w-4" /></button>
                      </div>
                      <button disabled={busy === item.id} aria-label="Remove item" onClick={() => updateItem(item.id, 0)} className="grid h-10 w-10 place-items-center rounded-lg border border-[var(--color-border)] text-[var(--color-danger)]"><Trash2 aria-hidden className="h-4 w-4" /></button>
                    </div>
                  </div>
                  <p className="mt-2 text-xs text-[var(--color-success)]">{item.product.stock_quantity > 0 ? "In stock" : "Out of stock"}</p>
                </div>
              </article>
            );
          })}
        </div>
        <aside className="sticky bottom-[calc(84px+env(safe-area-inset-bottom))] rounded-lg border border-[var(--color-border)] bg-white p-4 shadow-xl md:bottom-4 lg:top-5 lg:self-start">
