...
export function Gallery({ gallery, videos, slides, viewer }: { gallery: ProductMedia[]; videos: ProductMedia[]; slides?: ProductMedia[]; viewer: Viewer360 }) {
  ...
  return (
    <section>
      ...
      <div className="relative aspect-[4/5] overflow-hidden bg-white lg:max-h-[760px]">
        ...
        {mode === "gallery" ? <button onClick={() => setLightboxOpen(true)} aria-label="Open media fullscreen" className="absolute right-3 top-3 inline-flex h-10 items-center gap-2 rounded-full border border-[var(--color-border-strong)] bg-white/95 px-4 text-sm font-bold text-[var(--color-text)]"><Maximize2 aria-hidden className="h-4 w-4" />View image · {selectedIndex + 1}/{media.length}</button> : null}
      </div>
