...
export function Viewer360Panel({ viewer }: { viewer: Viewer360 }) {
  ...
  const shell = fullscreen ? "fixed inset-0 z-[100] bg-white" : "relative h-full w-full";
  ...
  if (viewer.mode === "model") {
    return (
      <div role={fullscreen ? "dialog" : undefined} aria-modal={fullscreen || undefined} aria-label={fullscreen ? "Fullscreen 3D viewer" : undefined} className={shell}>
        ...
        <button onClick={() => setFullscreen((value) => !value)} aria-label={fullscreen ? "Close fullscreen 3D viewer" : "Fullscreen 3D viewer"} className="absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-full border border-[var(--color-border)] bg-white/90">{fullscreen ? <Minimize2 aria-hidden className="h-4 w-4" /> : <Maximize2 aria-hidden className="h-4 w-4" />}</button>
      </div>
    );
  }
  return (
    <div
      ...
      aria-label={fullscreen ? "Fullscreen 360 product viewer" : undefined}
      className={`${shell} cursor-grab touch-pan-y active:cursor-grabbing`}
      onPointerMove={(event) => {
    ...
    >
      ...
      <button onClick={() => setFullscreen((value) => !value)} aria-label={fullscreen ? "Close fullscreen 360 viewer" : "Fullscreen 360 viewer"} className="absolute right-3 top-16 grid h-10 w-10 place-items-center rounded-full border border-[var(--color-border)] bg-white/90">{fullscreen ? <Minimize2 aria-hidden className="h-4 w-4" /> : <Maximize2 aria-hidden className="h-4 w-4" />}</button>
    </div>
  );
}
